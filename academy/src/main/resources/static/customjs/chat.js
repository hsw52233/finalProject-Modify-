		  ...
		  $.ajax({
		    ...
		    success: (data) => {
		      ...
		      this.searchUsers = data
		        .filter(item => {
		          // item[1] (userId)가 null 또는 undefined인 경우 제외
		          return item[1] && parseInt(item[1]) !== parseInt(this.currentUserId);
		        })
		        .map(item => {
		          return {
		            userId: item[1] || 'unknown',  // item[1]이 null일 경우 'unknown' 처리
		            name: item[0] || 'No Name',  // item[0]이 null일 경우 'No Name' 처리
		            path: item[5] && item[5] !== 'null.null' ? item[5] : 'defaultProfile.png'  // item[5]가 null이면 기본 프로필 이미지로 처리
		          };
		        });
